...
void square(int secs=2, int turnSpeed=45)
{
    ...
    { // Rij vooruit
        ...
        sleep(secs);
        // ga naar rechts
        BP.set_motor_dps(PORT_B, -turnSpeed);
        BP.set_motor_dps(PORT_C, turnSpeed);
        usleep(700000);
        stop();
    }
}
...
int main(){
  ...
  while(true){
	...
	if(BP.get_sensor(PORT_1, Color1) == 0 && BP.get_sensor(PORT_3, Light3) == 0){
		...
		else if(Color1.ambient <540 && Color1.ambient>500){
			if(prev !=2){
			stop();
			//usleep(100000);
			}
